...
if (window.location.pathname.toLowerCase() !== '/account/login') {
    var token = localStorage.getItem('aia_jwt_token');
    if (!token) {
...
(function ($, app) {
        ...
        if (window.location.pathname !== '/account/signin' && window.location.pathname !== '/account/verification') {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('.gmia-data-username').text(data.content.userName || data.content.fullName);
                    $('.gmia-data-rolename').text(data.content.roleName);
                    $('.gmia-data-mobile').text(data.content.mobilePhone || '');
                    $('.gmia-data-email').text(data.content.employeeEmail || data.content.email || '');
    ...
    app.utility.webAjaxWithToken = function (url, type, param, options) {
        ...
        const headers = {
            ...
            'Authorization': 'Bearer ' + localStorage.getItem('aia_jwt_token')
        };
    ...
    app.security.isAdministrator = function () {
        ...
        if (userInfoRaw) {
            ...
            // In this system: 0 = Supervisor (Admin), 1 = Employee
            if (roleId === '0' || String(role).toLowerCase() === 'supervisor') {
                return true;
            }
        }
        return false;
    ...
    app.security.isFinance = function () {
        ...
        if (userInfoRaw) {
            ...
            // Adapt as needed if a specific Finance role ID is added to SystemLookup later
            // For now, only Supervisor (0) has elevated permissions
            if (roleId === '0') {
                return true;
            }
        }
