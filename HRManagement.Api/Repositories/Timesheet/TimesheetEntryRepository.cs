...
public class TimesheetEntryRepository(AppDbContext dbContext) 
    : BaseRepository<TimesheetEntry>(dbContext), ITimesheetEntryRepository
{
    ...
    private async Task HydrateEntriesAsync(IEnumerable<TimesheetEntry> entries)
    {
        ...
        var lds = await dbContext.Employees.Where(e => lIds.Contains(e.Id)).ToListAsync();
        var emp = await dbContext.Employees.Where(e => eIds.Contains(e.Id)).ToListAsync();
