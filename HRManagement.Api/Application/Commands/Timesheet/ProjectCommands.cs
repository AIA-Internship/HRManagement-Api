            ...
            CancellationToken cancellationToken)
        {
            ...
            foreach (var item in dto.Projects)
            {
                if (item.Id.HasValue && item.Id > 0)
                {
                    ...
                    if (project != null)
                    {
                        project.UpdateDetails(item.ProjectName, null, item.ProjectLeader, 0, actionerId);
                        await projectRepository.UpdateAsync(project);
                    }
                }
                else
                {
                    // Create new
                    var newProject = new TimesheetProject(item.ProjectName, null, item.ProjectLeader, actionerId);
                    try
                    {
