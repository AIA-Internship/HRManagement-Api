...
public class WeeklyProjectRowDto
{
    public int ProjectId { get; set; }
    public string ProjectName { get; set; } = string.Empty;

    /// <summary>Key = date string (yyyy-MM-dd), Value = duration in minutes.</summary>
    public Dictionary<string, int> DailyMinutes { get; set; } = new();
    public int WeeklyTotalMinutes { get; set; }
    public string WeeklyTotalFormatted { get; set; } = string.Empty;
}
...
public class WeeklyTimesheetResponseDto
{
    ...
    public List<WeeklyProjectRowDto> Projects { get; set; } = new();
}
