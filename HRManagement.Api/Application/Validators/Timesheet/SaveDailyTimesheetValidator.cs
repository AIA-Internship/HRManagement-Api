...
public class SaveDailyTimesheetValidator : AbstractValidator<SaveDailyTimesheetCommand>
{
    public SaveDailyTimesheetValidator()
    {
        ...
        RuleFor(x => x.RequestDto.Entries)
            ...
            .NotEmpty().WithMessage("At least one entry row is required.");

        RuleForEach(x => x.RequestDto.Entries).ChildRules(entry =>
        {
            ...
            entry.RuleFor(e => e.ApplicationUsed)
                .NotEmpty().WithMessage("Application used is required.")
                .MaximumLength(300).WithMessage("Application used cannot exceed 300 characters.");
            ...
            entry.RuleFor(e => e.ProjectLeadId)
                .GreaterThan(0).WithMessage("Please select a valid project lead.");
